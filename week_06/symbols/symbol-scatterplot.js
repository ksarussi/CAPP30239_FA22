...
(function symbol() {
  ...
  d3.csv('penguins.csv').then(data => {
    ...
    svg.append("g")
        .selectAll(".cross")
        .data(data)
        .join("path") // here we join on a path (symbols are paths, not circles)
        .attr("d", d3.symbol().type(d3.symbolCross).size(12)) // pick size of symbol
        .attr("class", "cross") // "d" in quotes is the "d" that goes on html. d3. is the path it uses
        .attr("fill", "orange")
        .attr("opacity", 0.75)
    ...
    d3.selectAll(".cross") // note we are selecting all the symbols by class
      ...
      .on("mousemove", function(event) {
        tooltip
          .style("top", (event.pageY - 10) + "px") // detect event page (event - get the value of teh select)
          .style("left", (event.pageX + 10) + "px"); // gets the x and y position of our mouse
      })
